...
**문제**: 스페셜리스트 직급으로 성별 구매 분석, 연령대별 성별 구매 차트 데이터 조회 불가

**원인**: `get_user_advertiser_ids` PostgreSQL 함수에서 `specialist` 역할 누락

**해결**: 함수 수정하여 `specialist`도 `master`와 동일하게 모든 광고주 접근 허용
