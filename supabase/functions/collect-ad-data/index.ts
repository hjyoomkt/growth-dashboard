...
serve(async (req) => {
  ...
  try {
    ...
    // 6. 날짜 청크 계산
    const chunks = calculateDateChunks(start_date, end_date, chunkSizeDays)
...
function calculateDateChunks(startDate: string, endDate: string, chunkSizeDays: number): { start: string; end: string }[] {
