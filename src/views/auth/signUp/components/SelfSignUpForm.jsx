...
function SelfSignUpForm({ onSuccess }) {
  const [formData, setFormData] = useState({
    ...
    confirmPassword: "",
    // 조직 정보
    organizationType: "advertiser", // advertiser 또는 agency
    organizationName: "",
    businessNumber: "",
    contactEmail: "",
    contactPhone: "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: existingOrg } = await supabase
        .from('organizations')
        .select('id, name')
        .or(`name.eq.${formData.organizationName},business_number.eq.${formData.businessNumber}`)
        .limit(1);

      if (existingOrg && existingOrg.length > 0) {
        setError(`"${existingOrg[0].name}" 조직이 이미 존재합니다. 초대 코드를 받으셨나요?`);
        return;
      }
      ...
      const { data: orgData, error: orgError } = await supabase
        ...
        .insert({
          ...
          max_advertisers: formData.organizationType === 'advertiser' ? 1 : 999,
          is_active: true, // Phase 2에서는 false로 변경 (승인 필요)
        })
        .select()
        .single();
      ...
      // 4. Advertisers 테이블에 광고주 생성 (advertiser 타입인 경우)
      let advertiserId = null;
      if (formData.organizationType === 'advertiser') {
        const { data: advData, error: advError } = await supabase
          .from('advertisers')
          .insert({
            organization_id: orgData.id,
            name: formData.organizationName, // 조직명과 동일
            business_number: formData.businessNumber,
            contact_email: formData.contactEmail || formData.email,
            contact_phone: formData.contactPhone,
          })
        ...
        advertiserId = advData.id;
      }

      // 5. Users 테이블에 사용자 생성 (org_admin 권한)
      const { error: userError } = await supabase
        .from('users')
        .insert({
          id: authData.user.id,
          organization_id: orgData.id,
          advertiser_id: advertiserId, // agency는 null, advertiser는 ID
          email: formData.email,
          name: formData.name,
          role: 'org_admin', // 첫 사용자는 조직 관리자
        });
