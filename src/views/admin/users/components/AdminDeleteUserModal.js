...
export default function AdminDeleteUserModal({
  ...
  // advertiser_admin인 경우 브랜드 사용자 목록 조회
  useEffect(() => {
  ...
  const fetchBrandUsers = async () => {
    ...
    try {
      ...
      console.log('[AdminDeleteUserModal] Brand users loaded:', users);
      setBrandUsers(users || []);
    } catch (error) {
  ...
  const handleDelete = async () => {
    // advertiser_admin인데 소유권 이전 대상이 없는 경우
    ...
    // advertiser_admin인데 새 소유자를 선택하지 않은 경우
          ...
          <VStack align="stretch" spacing={4}>
            ...
            <VStack align="stretch" spacing={2} pl={4}>
              <Text fontSize="sm" color={textColor}>
                • API 토큰 (브랜드 소유, 계속 사용 가능)
              </Text>
              <Text fontSize="sm" color={textColor}>
                • 광고 성과 데이터
              </Text>
              <Text fontSize="sm" color={textColor}>
                • 작성한 게시글 (작성자는 "알 수 없음"으로 표시)
              </Text>
            </VStack>
